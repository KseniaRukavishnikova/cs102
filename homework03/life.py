...
class GameOfLife:
    ...
    def cell_list(self, randomize: bool=True) -> Clist:
        ...
        self.clist = []
        if randomize:
            self.clist = [[random.randint(0, 1) for i in range(int(self.cell_width))] for j in range(int(self.cell_height))]
        else:
            self.clist = [[0 for i in range(int(self.cell_width))]for j in range(int(self.cell_height))]
        return self.clist
